...
async function suggestClients(query, opts = {}) {
  const { includeArchived = false } = opts;
  const q = (query || '').trim().toLowerCase();
  const clients = await getClients();
  const all = clients.filter(c => includeArchived ? true : !c.archived);
...
async function routeAutocomplete(interaction) {
  ...
  if (cmd === 'job') {
    if (sub === 'create') {
      if (focusedName === 'client') return interaction.respond(await suggestClients(focusedValue));
      if (focusedName === 'tags')   return interaction.respond(await suggestTags(focusedValue));
    }

    if (sub === 'edit') {
      if (focusedName === 'id')     return interaction.respond(await suggestJobs(focusedValue));
      if (focusedName === 'client') return interaction.respond(await suggestClients(focusedValue));
      if (focusedName === 'tags')   return interaction.respond(await suggestTags(focusedValue));
    }
    ...
    if (sub === 'delete') {
      if (focusedName === 'id')     return interaction.respond(await suggestJobs(focusedValue)); // ✅ new
    }

    if (sub === 'list') {
      if (focusedName === 'client') return interaction.respond(await suggestClients(focusedValue));
      if (focusedName === 'tag')    return interaction.respond(await suggestTags(focusedValue));
    }
  }
  ...
  if (cmd === 'client') {
    if (group === 'panel' && sub === 'set') {
      if (focusedName === 'id')     return interaction.respond(await suggestClients(focusedValue));
    }
    if (sub === 'edit') {
      if (focusedName === 'id')     return interaction.respond(await suggestClients(focusedValue));
    }
    if (sub === 'merge') {
      if (focusedName === 'id1' || focusedName === 'id2') {
        return interaction.respond(await suggestClients(focusedValue));
      }
    }
    if (sub === 'delete') {
      if (focusedName === 'id')     return interaction.respond(await suggestClients(focusedValue, { includeArchived: true })); // ✅ include archived
    }
  }
