...
async function ensureClientChannel(client, guildId, clientRec) {
  ...
  if (!channel) {
    ...
    // Sync permissions with parent category (skip if no permission)
    try {
      await channel.lockPermissions();
      console.log(`🔐 Synced permissions for channel: ${channel.name}`);
    } catch (permError) {
      console.warn(`⚠️ Could not sync channel permissions (continuing anyway): ${permError.message}`);
    }
    clientRec.channelId = channel.id;
...
function buildClientCardEmbed(clientRec, jobs, guildId) {
  ...
  descriptionParts.push(
    notesLine,
    '═══════════════════════════════════', // Horizontal divider after notes
    '',
    '**Open Jobs:**',
    '', // Line break after "Open Jobs:"
