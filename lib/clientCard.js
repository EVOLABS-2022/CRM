...
function buildClientCardEmbed(clientRec, jobs, guildId) {
  ...
  const contactBits = [clientRec.contact, clientRec.email, clientRec.phone].filter(Boolean).join(' | ');
  ...
  return new EmbedBuilder()
    ...
    .setFooter({ text: `Client ID: ${clientRec.id}` });
}
...
async function syncAllClientChannelsAndCards(client, guildId) {
  ...
  for (const c of clients) {
    try { await ensureClientCard(client, guildId, c); } catch {}
  }
}
