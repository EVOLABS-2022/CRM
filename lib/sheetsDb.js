...
async function createClient(clientData) {
  ...
  const row = [
    ...
    'yes' // Column L: Active - new clients created via Discord are automatically active
  ];
...
async function getClients() {
  ...
  try {
    ...
    return rows.slice(1).map(row => ({
      ...
      notes: row[9] || '',
      createdAt: row[10] || '',
      active: row[11] || '' // Column L: Active status
    })).filter(client => client.name && client.name.trim() !== ''); // Filter out rows without names (leads need names, not necessarily IDs)
  } catch (error) {
...
async function updateClientChannel(clientId, channelId, messageId) {
  ...
  // If no match by ID, try to find by name (for converted leads)
  if (clientIndex === -1 && typeof clientId === 'string') {
    clientIndex = clients.findIndex(c => c.name === clientId);
  }
  
  if (clientIndex === -1) {
    console.error(`❌ Client not found for channel update: ${clientId}`);
    return false;
  }
...
async function updateClient(clientId, updates) {
  ...
  // If no match by ID, try to find by name (for leads without IDs)
  if (clientIndex === -1 && typeof clientId === 'string') {
    clientIndex = clients.findIndex(c => c.name === clientId);
  }
  ...
  const row = [
    ...
    updatedClient.active || '' // Column L: Active status
  ];
  
  await api.spreadsheets.values.update({
    spreadsheetId: SPREADSHEET_ID,
    range: `Clients!A${rowNumber}:L${rowNumber}`,
    valueInputOption: 'RAW',
    requestBody: {
      values: [row]
    }
