...
async function createClient(clientData) {
  ...
  // Invalidate clients cache since we added new data
  invalidateCache('clients');
  
  return clientData;
}

// Original function (used by cache)
async function originalGetClients() {
  const api = await getSheets();
...
async function updateClientChannel(clientId, channelId, messageId) {
  ...
  // Invalidate clients cache since we updated channel info
  invalidateCache('clients');
...
async function updateClient(clientId, updates) {
  ...
  // Invalidate clients cache since we updated data
  invalidateCache('clients');
...
async function createJob(jobData) {
  ...
  // Invalidate jobs cache since we added new data  
...
// Original function (used by cache)  
async function originalGetJobs() {
  const api = await getSheets();
...
async function updateJobThread(jobId, threadId, messageId) {
  ...
  // Invalidate jobs cache since we updated thread info
  invalidateCache('jobs');
...
async function updateJob(jobId, updates) {
  ...
  // Invalidate jobs cache since we updated data
  invalidateCache('jobs');
...
async function createInvoice(invoiceData) {
  ...
  // Invalidate invoices cache since we added new data
...
// Original function (used by cache)
async function originalGetInvoices() {
  const api = await getSheets();
...
async function updateInvoice(invoiceId, updates) {
  ...
  // Invalidate invoices cache since we updated data
  invalidateCache('invoices');
...
async function createTask(taskData) {
  ...
  // Invalidate tasks cache since we added new data
...
// Original function (used by cache)
async function originalGetTasks() {
  const api = await getSheets();
...
async function updateTask(taskId, updates) {
  ...
  // Invalidate tasks cache since we updated data
  invalidateCache('tasks');
...
async function cleanupTasksForClosedJobs() {
  ...
  // Invalidate tasks cache since we deleted tasks
  if (tasksToDelete.length > 0) {
    invalidateCache('tasks');
  }
...
// === CACHE SETUP ===
// Setup the cache with original functions
setOriginalFunctions({
  getClients: originalGetClients,
  getJobs: originalGetJobs,
  getInvoices: originalGetInvoices,
  getTasks: originalGetTasks
});

// New cached getter functions (these replace the originals in exports)
