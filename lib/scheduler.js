// lib/scheduler.js
  ...
  // const { refreshTasksChannel } = require('../utils/tasksChannel'); // TEMP REMOVE
  ...
  async function runSyncTick(client) {
    ...
    try {
      ...
      await Promise.all([
        refreshInvoicesBoard(client, invoices, clients, jobs)
        // refreshTasksChannel(client, client.guilds.cache.keys().next().value) // TEMP REMOVE
      ]);
