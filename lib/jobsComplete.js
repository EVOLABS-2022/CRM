...
// TEMP fallback so you're unblocked if env isn’t wired yet
const FALLBACK_SHEET_ID = '1abfOFq4Sh8MXgS3ubPa-n2uuRS6eRXeWIPQvP07jwZk';

// --- DIAGNOSTIC: prove the file/version/env at runtime
const VERSION = 'vdiag-3';
console.log(`[jobsComplete] LOADED ${VERSION}`);
console.log(`[jobsComplete] env.GOOGLE_SHEETS_ID prefix: ${
  (process.env.GOOGLE_SHEETS_ID || '').slice(0, 8) || '(unset)'
}`);
...
function resolveSpreadsheetId() {
  ...
  // --- DIAGNOSTIC: show where the ID came from
  console.log(`[jobsComplete] spreadsheetId source: ${
    fromModule ? 'sheetsDb' : (fromEnv ? 'env' : 'FALLBACK')
  } -> ${id}`);
...
async function setJobComplete(jobId) {
  ...
  console.log(`[jobsComplete] writing ${statusA1} = "Complete"`);
