...
async function getExistingJobFolders(clientFolderId) {
  try {
    ...
    // Filter folders that look like job codes (format: XXXX-001, XXXX -001, etc - allowing spaces before dash)
    const jobFolders = response.data.files.filter(folder => {
      return /^[A-Z0-9]{3,4}\s*-\d{3}$/.test(folder.name.trim());
    });
...
async function syncJobFolders(clients, jobs) {
  ...
  // Debug: Print all job data
  console.log('🔍 Job debugging:');
  jobs.forEach((job, index) => {
    console.log(`   Job ${index + 1}: title="${job.title}", code="${job.code}", id="${job.id}"`);
  });
  ...
  try {
    ...
    for (const job of jobs) {
      // Use job.id as the job code (that's where the code is actually stored)
      const jobCode = job.id;
      if (!jobCode || jobCode.trim() === '') {
        console.warn(`⚠️ Job "${job.title}" has no job code (id: "${job.id}"), skipping folder creation`);
        continue;
      }
      ...
      if (!client || !client.code) {
        console.warn(`⚠️ Job ${jobCode} has no valid client, skipping folder creation`);
        continue;
      }
      
      // Get the client folder ID (trim the client code)
      const clientFolderId = await getClientFolderId(client.code.trim());
      if (!clientFolderId) {
        console.warn(`⚠️ Client folder not found for ${client.code.trim()}, skipping job folder creation`);
        continue;
      }
      ...
      console.log(`🔍 Checking if job folder ${jobCode.trim()} exists in existing: [${Array.from(existingJobCodes).join(', ')}]`);
      
      if (!existingJobCodes.has(jobCode.trim())) {
        console.log(`📁 Creating missing job folder for ${jobCode} - ${job.title}`);
        const folderId = await createJobFolder(clientFolderId, jobCode, job.title);
        if (folderId) {
          created++;
        } else {
          errors++;
        ...
        console.log(`✅ Job folder ${jobCode.trim()} already exists, skipping`);
        skipped++;
      }
    }
